...
// When Claude names no croppable area, fall back to the two areas Endolift treats
// most commonly so the user still gets a focused, on-device look.
const DEFAULT_REGIONS: RegionKey[] = ["jawline", "chin"];
...
/**
 * Shows the user their OWN selfie, cropped to the lower-face areas Endolift
 * targets, with a short "what it does here" line per area. Everything is computed
  ...
  return (
    <div>
      ...
      <p className="mx-auto mt-1 max-w-md text-center text-[13px] leading-relaxed text-body/80">
        Taken from your own photo, on your device. A focused look at where
        Endolift could work for you — confirmed at your consultation.
      </p>
        ...
        {crops.map((c, i) => {
          ...
          return (
            <motion.div
              ...
              className="overflow-hidden rounded-[1.5rem] border border-peach/30 bg-white/70 shadow-soft"
            >
              <div className="relative aspect-[4/3] w-full overflow-hidden bg-heading/10">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={c.src}
                  alt={`Your ${c.region}, the area Endolift would target`}
                ...
                <span className="absolute left-3 top-3 rounded-full bg-heading/65 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-white backdrop-blur-sm">
                  {copy.title}
                </span>
              </div>
